...
BREAKOUT_RSI_MIN = 55
WATCHING_DISTANCE_PCT = 1.5
WATCHING_VOLUME_RATIO = 1.1
...
def analyze(candles):
    ...
    signal = "neutral"
    if last_close > resistance and vol_ratio and vol_ratio >= BREAKOUT_VOLUME_RATIO and (rsi_val or 0) > BREAKOUT_RSI_MIN:
        signal = "breakout"
    elif 0 <= dist_pct < WATCHING_DISTANCE_PCT and vol_ratio and vol_ratio >= WATCHING_VOLUME_RATIO:
        signal = "watching"
...
def notify(product_id, result):
    ...
    text = (
        ...
        f"מחיר: {result['last_close']:.6g}\n"
        f"התנגדות: {result['resistance']:.6g}\n"
        f"נפח יחסי: {result['vol_ratio']:.2f}x\n"
        f"RSI: {result['rsi']:.0f}"
    )
